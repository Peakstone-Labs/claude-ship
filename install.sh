...
mkdir -p "$DEST/commands" "$DEST/agents" "$DEST/templates/development" "$DEST/scripts" "$DEST/third-party-review.d"
...
for ex in "$SRC"/third-party-review.d/*.env.example; do
  [ -e "$ex" ] || continue
  cp -vn "$ex" "$DEST/third-party-review.d/"
done

echo
echo "Done."
echo "  • Slash commands: /clarify /architect /third_party_review /ship /retro (and dev/review/qa run inside /ship)"
echo "  • To enable /third_party_review: copy third-party-review.d/provider.env.example"
